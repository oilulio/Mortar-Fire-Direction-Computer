...
// Mortar flight calculation is pure example.

// V1.0 functionality
// Resection
// Set ML, including temperature and elevation
// Enter target.  
// Shot at target with countdown, and min/max range checks
// Keyboard and screen input, including back button and Run on enter.

// V1.1 Changes:
// Squared up image

// V1.2 Added functionality/fixes:
// Added tooltips for known buttons
// H hour setting/countdown/reset
// Actual time setting (by default uses host time)
// Decimal point enabled (needed for actual time)
// Clear button functionality
// SIM button (within limits of my understanding of its function)
// Back button correctly deals with deleting last digit (i.e. not leaving a minus standing alone)
// WM button - snap shot at range-bearing target
// Blank entry now reports "INPUT REQ'D"
...
const COUNTDOWN=21;
const CLEAR_ACTIVE=22;
const CLEAR_CONFIRM=23;
const SET_H_HR=24;
const H_HR_COUNTDOWN=25;
const H_HR_COUNTUP=26;
const GET_TGT_BRG=27;
const GET_TGT_RNG=28;
const SETTING_TIME=29;
const SETTING_MET=30;
...
function onKeyDown(evt){   // Give me some keyboard control (Enter, backspace,plus/minus and 0-9.)
  ...
  else if (evt.keyCode==190) {Bdp(); return false; }
}
...
var c = document.getElementById("myCanvas");
var ctx = c.getContext("2d");
ctx.clearRect(222,253,90,89);
ctx.drawImage(img,offsetX,offsetY,540,1000);
}
function outDATUM() {
var c = document.getElementById("myCanvas");
var ctx = c.getContext("2d");
ctx.clearRect(314,253,90,89);
ctx.drawImage(img,offsetX,offsetY,540,1000);
}
function outTgts() {
var c = document.getElementById("myCanvas");
...
var c = document.getElementById("myCanvas");
var ctx = c.getContext("2d");
ctx.clearRect(42,709,88,89);
ctx.drawImage(img,offsetX,offsetY,540,1000);
}
function outCLEAR() {
var c = document.getElementById("myCanvas");
var ctx = c.getContext("2d");
ctx.clearRect(42,799,88,89);
ctx.drawImage(img,offsetX,offsetY,540,1000);
}
function outRESET() {
var c = document.getElementById("myCanvas");
var ctx = c.getContext("2d");
ctx.clearRect(42,889,88,89);
ctx.drawImage(img,offsetX,offsetY,540,1000);
}
function outB0() {
var c = document.getElementById("myCanvas");
...
if (state!=H_HR_COUNTDOWN) return;

h_secs-=1;
if (h_secs==0 && h_mins==0 && h_hrs==0) {
  document.getElementById('maintext').textContent="* H HOUR *"; 
  setTimeout(Hcountup,1000);
  state=H_HR_COUNTUP;
  return false;
}
if (h_secs<0) {
...
document.getElementById('maintext').textContent="H-"+h_hrs.toString().padStart(2,'0')+":"+h_mins.toString().padStart(2,'0')+":"+h_secs.toString().padStart(2,'0'); 
setTimeout(Hcountdown,1000);
...
if (keyboardInput.length>0) {
  ...
  if (keyboardInput=='-') keyboardInput=""; // Special case - don't just leave a minus on display
  document.getElementById('maintext').textContent=keyboardInput;
}
if (keyboardInput.length==0) {
...
if (state==CLEAR_ACTIVE) { 
  document.getElementById('maintext').textContent="RUN IF SURE"; 
  state=CLEAR_CONFIRM;
  return FALSE;
}
 
// Seems to toggle missions.  If both true, sets to 1
// If neither, does nothing (confirm?)
if      (mission2) { mission2=false; mission1=true; }
else if (mission1) { mission1=false; mission2=true; }
showMissions();

keyboardInput="";
document.getElementById('maintext').textContent="MISSION  "+(mission1?"1":"2"); 
setTimeout(confirmTemp,1000);
state=READY; 
...
if (state==CLEAR_ACTIVE) {
  document.getElementById('maintext').textContent="MSN CLEARED"; 
...
keyboardInput="";
document.getElementById('maintext').textContent="NO TARGETS"; 
setTimeout(confirmTemp,1000);
state=READY; 
...
// TODO - Implement whatever it does ...
...
if (state==CLEAR_ACTIVE) { // Can't press it twice
  document.getElementById('maintext').textContent=INVALID_KEY; 
  state=READY;
  setTimeout(backToReady,1000);
  return false;
...
state=CLEAR_ACTIVE; 
keyboardInput="";
document.getElementById('maintext').textContent="CLEAR _"; 
...
//------------------------------------------------------------------
function showMissions() { // Not fully understood - the "12" at bottom of display

document.getElementById('one').textContent=(mission1)?"1":""; 
document.getElementById('two').textContent=(mission2)?"2":""; 
...
if (shift) {
  Shift();  // Cancel shift
  document.getElementById('maintext').textContent="MET GROUP 3?"; 
  state=SETTING_MET;
} else {
  document.getElementById('maintext').textContent="STANDARD MET"; 
  state=READY;
  setTimeout(backToReady,1000);
}
return false;
...
//------------------------------------------------------------------
function WM() { // Seems to be a range-bearing shot from unsurveyed location
...
keyboardInput=""; // Cleanup
state=GET_TGT_BRG;  
document.getElementById('maintext').textContent="ML-TGT BG ?"; 
return false;
}
//------------------------------------------------------------------
function Shot() {
...
if (tgt_range!=-1) distance=tgt_range;  // the range-brg shot
...
if (shift) { // It is clock setting time
  Shift();  //Unset it
  keyboardInput="";
  document.getElementById('maintext').textContent="TIME? HHMM.SS"; 
  state=SETTING_TIME;
...
if (shift) { // Set H-Hour
  keyboardInput="";
  document.getElementById('maintext').textContent="H HR ? HHMM"; 
  state=SET_H_HR;
  Shift();  // Cancel shift
  return false;
}  
...
if (shift) return false;  // Do nothing if shifted (would be self test, but pointless)

if (keyboardInput.length<12 && keyboardInput.indexOf(".")<0) {
  keyboardInput+=".";
  document.getElementById('maintext').textContent=keyboardInput;
}
return false;
...
if (state==CLEAR_CONFIRM) {
  document.getElementById('maintext').textContent="MSNS CLEARED"; 
  // I should doubtless actually clear something here ...
  state=READY;
  setTimeout(backToReady,1000); 
  return false; 
}

if (keyboardInput.length==0  && state!=TEMP_UPDATE && state!=SHOW_ML_ALT && state!=SHOW_ML_GRID && state!=SOLUTION && state!=SOLUTION_EL && state!=GET_TGT_BRG && state!=GET_TGT_RNG) {
...
switch (state) {
  case(TGT_GRID):  tgt_grid=getGrid(); 
    if (tgt_grid==-2)  { defaultText="STOTGT GRID?"; setTimeout(backToReady,1000); break; }
  ...
  case(GET_GRID):  tgt_grid=getGrid();   // Snap shot at a grid
    if (tgt_grid==-2)  { defaultText="TGT GRID?" ; setTimeout(backToReady,1000); break; }
  ...
  case(GET_ALT):  tgt_alt=Number(keyboardInput); 
    if (!isNaN(tgt_alt))  {
      state=SOLUTION; 
      document.getElementById('maintext').textContent="ML:00 HE/WP4"; // High Explosive/White Phosphorous 4?
  ...
  case (SET_H_HR):
    var tmp=Number(keyboardInput);
    if (keyboardInput.length!=4 || isNaN(tmp) || tmp<0 || ((tmp%100)>59) || 
        (Math.trunc(tmp/100)>23)) { // 24h clock.  Mins<59 Hrs<24
      document.getElementById('maintext').textContent="INPUT ERROR"; 
      setTimeout(backToReady,1000);
      state=READY;
      keyboardInput="";
      return false;
    }
    var tmp_mins=tmp%100;
    var tmp_hrs=Math.trunc(tmp/100);

    var today=new Date(); // Countdown is until next alignment of times, i.e. <=24h 
    var hr=today.getHours();
    var min=today.getMinutes();
    var sec=today.getSeconds();
    var timeNow=hr*3600.0+min*60.0+sec+timeOffset;
    if (timeNow>(24.0*3600)) timeNow-=(24.0*3600);
    sec=Math.trunc(timeNow%60);
    timeNow/=60;
    min=Math.trunc(timeNow%60);
    hr=Math.trunc(timeNow/60);

    h_hrs=tmp_hrs-hr;
    h_mins=tmp_mins-min;
    h_secs=60-sec;
    if (sec==0) { h_secs=0;} else { h_mins-=1; }

    if (h_secs==0 && h_mins==0 && h_hrs==0) { // Corner case
      document.getElementById('maintext').textContent="* H HOUR *"; 
      setTimeout(Hcountup,1000);
      state=H_HR_COUNTUP;
      break;
    ...
    if (h_mins<0) { h_hrs-=1; h_mins+=60; }
    if (h_hrs<0) h_hrs+=24;

    keyboardInput="";
    state=H_HR_COUNTDOWN;
    document.getElementById('maintext').textContent="H-"+h_hrs.toString().padStart(2,'0')+":"+h_mins.toString().padStart(2,'0')+":"+h_secs.toString().padStart(2,'0'); 
    setTimeout(Hcountdown,1000);
    break;
  case (SETTING_TIME):
    var tmp=Number(keyboardInput);
    var tmp_secs=(tmp*100)%100;
    var tmp_mins=tmp%100;
    var tmp_hrs=Math.trunc(tmp/100);
    if (keyboardInput.length!=7 || isNaN(tmp) || tmp_secs>59 || tmp_mins>59 || tmp_hrs>24 || tmp<0) { // 24h clock with seconds (HHMM.SS)
      document.getElementById('maintext').textContent="INPUT ERROR"; 
      setTimeout(backToReady,1000);
      state=READY;
      keyboardInput="";
    ...
    var timeOfDay=tmp_hrs*3600.0+tmp_mins*60.0+tmp_secs;
    var today=new Date(); // Countdown is until next alignment of times, i.e. <=24h 
    var hr=today.getHours();
    var min=today.getMinutes();
    var sec=today.getSeconds();
    var trueTimeOfDay=hr*3600.0+min*60.0+sec;

    timeOffset=(timeOfDay-trueTimeOfDay); // Capped < 24h
    if (timeOffset<0) timeOffset+=24.0*3600.0;

    document.getElementById('maintext').textContent="CLOCK RESET"; 
    setTimeout(backToReady,1000);
    state=READY;
    keyboardInput="";
    return false;
  ...
  case (SETTING_MET):
    var tmp=Number(keyboardInput);
    if (isNaN(tmp) || tmp!=4) { // Strangely anything but 4 is Input Error ...
      document.getElementById('maintext').textContent="INPUT ERROR"; 
      setTimeout(backToReady,1000);
      state=READY;
      keyboardInput="";
    ...
    document.getElementById('maintext').textContent="NO GROUP 4"; // ... and 4 easn't much good either
    setTimeout(backToReady,1000);
    state=READY;
    keyboardInput="";
    return false;
  ...
  case (GET_TGT_BRG):
    tgt_brg=getBrg();   // Snap shot at a bearing/range
    if (tgt_brg==-2)  { defaultText="ML-TGT BG ?"; setTimeout(backToReady,1000); break; }
    if (tgt_brg!=-1)  { state=GET_TGT_RNG; document.getElementById('maintext').textContent="ML-TGT RNG ?"; } break;
    break;
  case (GET_TGT_RNG):
    tgt_range=getRange();   
    if (tgt_range==-2)  { defaultText="ML-TGT RNG ?"; setTimeout(backToReady,1000); break; }
    if (tgt_range!=-1)  { 
      state=SOLUTION; // Follow same path as GRID key press, except need no ML
      document.getElementById('maintext').textContent="ML:00 HE/WP4"; // High Explosive/White Phosphorous 4?
      setTimeout(solution,1000);
    }
    break;
...
function getGrid() {
  ...
  if (keyboardInput.length==0) {
    document.getElementById('maintext').textContent="INPUT REQ'D";
    return -2;
  }
